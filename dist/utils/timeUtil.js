class TimeUtil {
    formatDuring(time, options) {
        if (time === null || time === undefined)
            ...
            return "";
        let ms = 1, s = 1000, m = s * 60, h = m * 60, d = h * 24, mm = d * 30, y = d * 365, units = ["y", "mon", "d", "h", "m", "s", "ms"], labels = ["Year", "Month", "Day", "Hour", "Minute", "Second", "Millisecond"], labelsPlural = ["Years", "Months", "Days", "Hours", "Minutes", "Seconds", "Milliseconds"];
    ...
    formatDate(time, formatString = "short", dateFormat = "LOCAL") {
        ...
        if (formatString === "short") {
            if (dateFormat === "ISO8601")
                ...
                formatString = "DD Mon yyyy hh:mm:ss GMT";
            else
        ...
        let str = formatString, oneValue, MONTH_LONG = [
            "January",
            "February",
            "March",
            "April",
        ...
        if (str.indexOf("ddd") >= 0) {
            ...
            if (oneValue === 1 || oneValue === 21 || oneValue === 31)
                oneValue += "st";
            else if (oneValue === 2 || oneValue === 22)
                oneValue += "nd";
            else if (oneValue === 3 || oneValue === 23)
                oneValue += "rd";
            else
                oneValue += "th";
